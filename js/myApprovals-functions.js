...
$(document).ready(function(){
    ...
    $('.filter-section').submit(function(e){
        ...
        var url="myApprovals.php?action=list&"+params;
        window.location=url;
    });
    getAllExpIds();
});
    ...
    $('#expensesTableContainer').jtable('load', undefined, function(){
        ...
        $.each($rows,function(){
            ...
            if(record.Billable==0){//If the line is billable
                ...
                if(record.ExpenseTypeid==5||record.ExpenseTypeid==6){//If its a personal meal or a business meal
                    if(!nonBillable[record.ExpenseTypeid][record.Date]){//If the date is not created, it will create a new date
                        nonBillable[record.ExpenseTypeid][record.Date]=0;
                    }
                    nonBillable[record.ExpenseTypeid][record.Date]=nonBillable[record.ExpenseTypeid][record.Date]+record.AmountUS; //It will add the amount to that date
                }else{
                    nonBillable[record.ExpenseTypeid]["no-date"]=nonBillable[record.ExpenseTypeid]+record.AmountUS;//is not a meal, it will ignore the date
                }
            }else{
